...
void draw() {
  ...
  if (init_com==1 && graph_on==1) {
    ...
    if ((time-time3)>20 && ! toggleRead && ! toggleWrite && ! toggleSetSetting) {
      ...
      sendRequestMSP(requestMSP(MSP_HEADING));
      time3=time;
    }
    if (toggleReset) {
      toggleReset=false;
    ...
    if (toggleRead) {
      toggleRead=false;
      int[] requests = {MSP_AUX_COUNT, MSP_BOXNAMES, MSP_RC_TUNING, MSP_PID, MSP_AUX, MSP_MISC, MSP_IDENT, MSP_MOTOR_PINS }; // MSP_PIDNAMES
      sendRequestMSP(requestMSP(requests));
